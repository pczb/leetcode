            ...
            while (!queue.isEmpty()) {
                ...
                for (String preString : currentlevel) {
                    for (int i = 0; i < preString.length(); ++i) {
                        for (char j = 'a'; j <= 'z'; ++j) {
                            char[] tmpchar = preString.toCharArray();
                            tmpchar[i] = j;
                            String tmps = new String(tmpchar);
                            if (!preString.equals(start) && tmps.equals(end)) {
                                map.get(end).add(preString);
                                queue.offer(tmps);
                            } else if (!tmps.equals(preString) && dict.contains(tmps)) {
                                if (!queue.contains(tmps)) queue.offer(tmps);
                                map.get(tmps).add(preString);
                            }
                        }
                    }
                }
